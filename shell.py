    ...
    def run(self, connection, max_end_time, args=None):  # pylint: disable=too-many-locals
        ...
        try:
            ...
            with connection.test_connection() as test_connection:
                ...
                #START
                #Line to check if bash/sh/ash exists
                checku="bashu=\"$(ls -l /bin | grep -E '\\bbash\\b' -o)\" ; shu=\"$(ls -l /bin | grep -E '\\bsh\\b' -o)\" ; ashu=\"$(ls -l /bin | grep -E '\\bash\\b' -o)\""
                self.logger.debug(checku);
                #bashu="%s/bin/lava-test-runner %s/%s" % ( lava_test_results_dir, lava_test_results_dir, running)
                #shu="sh %s/bin/lava-test-runner %s/%s" % ( lava_test_results_dir, lava_test_results_dir, running)
                #ashu="ash %s/bin/lava-test-runner %s/%s" % ( lava_test_results_dir, lava_test_results_dir, running)
                
                #If statement to create bash that runs ash/sh depending on what the image has in it
                ifu="if [ -n \"$bashu\" ]; then echo \"Bash exists\" ; elif [ -n \"$shu\" ]; then echo \"Sh exists\"; elif [ -n \"ashu\" ]; then echo \"Ash exists\" ; fi"
                createbashsh="echo $'#/bin/sh \\n\\nsh $*' > /bin/bash"
                createbashash="echo $'#/bin/ash \\n\\nash $*' > /bin/bash"
                self.logger.debug(createbashsh)
                self.logger.debug(createbashash)
                if_create="if [ -n \"$bashu\" ]; then echo \"Bash exists\" ; elif [ -n \"$shu\" ]; then %s; chmod 700 /bin/bash;elif [ -n \"ashu\" ]; then %s ; chmod 700 /bin/bash ;fi" % (createbashsh, createbashash)
                #ifu_two="if [ -n \"$bashu\" ]; then %s ; elif [ -n \"$shu\" ]; then %s; elif [ -n \"ashu\" ]; then %s ; fi" % (bashu, shu, ashu)
                self.logger.debug(ifu)
                self.logger.debug(if_create)
                #self.logger.debug(ifu_two)
                test_connection.sendline(checku)
                test_connection.sendline(ifu)
                test_connection.sendline(if_create)
                #test_connection.sendline("chmod 700 /bin/bash")
                #test_connection.sendline(ifu_two, delay=self.character_delay)
                #END
