...
class GitHelper(VCSHelper):
    ...
    def clone(self, dest_path, shallow=False, revision=None, branch=None):
        ...
        homu = subprocess.check_output(['bash', '-c' , 'echo "$HOME"'])
        logger.debug("$HOME is: ")
        logger.debug(homu)

        #logger.debug(homu_set)

        prokshi = subprocess.check_output(["git", "config", "--global", "https.proxy"])
        logger.debug("Proxy is: ")
        logger.debug(prokshi)
        #END OF NEW STUFF
