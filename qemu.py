...
class CallQemuAction(Action):
    ...
    def run(self, connection, max_end_time, args=None):
        ...
        if 'qemu-nfs' in self.methods and self.parameters.get('media', None) == 'nfs':
            ...
            shell_precommand_list.append('export TEST=`blkid | grep LAVA | cut -c6-8`')
            shell_precommand_list.append("ln -s ../../$TEST /dev/disk/by-uuid/$(blkid | grep LAVA | grep -E '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}' -o)")
